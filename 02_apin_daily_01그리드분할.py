...
■ 전체 흐름 요약
  ...
  [Step 2] 일별 B/S → 종목별 60일 롤링 APIN 추정 (Duarte & Young 2009 모델)
    ...
    - 60행 슬라이딩 윈도우로 순회하며 MLE로 APIN 추정
...
■ MLE 추정 방식
  10개 파라미터(α, δ, θ₁, θ₂, μ_b, μ_s, ε_b, ε_s, Δ_b, Δ_s)를 최대우도추정법으로 구한다.
  최적화는 2단계로 진행한다.

    [1단계] 그리드 탐색 — 3단계 최적화 전략 (메모리 + 속도)
      APIN은 10개 파라미터이므로 3^10 = 59,049개 그리드를 탐색해야 한다.
      PIN(G=243)과 달리 그리드가 243배 크므로, 메모리와 속도를 동시에 최적화한다.

        (a) 사전 필터링: 각 그리드의 모델 기대값 E[B], E[S]를 관측 평균과 비교하여
            명백하게 맞지 않는 후보를 제거한다. (1D 연산으로 < 1ms)
            → 59,049 → 약 5,000~9,000개로 축소 (이후 NLL 계산량 7~10배 감소)

        (b) 청크 분할: 필터링된 그리드를 8,192행 단위로 나눠 순회한다.
            피크 메모리가 그리드 전체 크기가 아닌 청크 크기에만 비례하므로
            워커당 메모리를 ~15 MiB 수준으로 제한할 수 있다.

        (c) 수동 logsumexp: 6개 시나리오를 np.stack으로 (6, C, N) 텐서로
            쌓는 대신, running max + running sum of exp로 하나씩 누적한다.
            3D 배열 할당이 완전히 제거되어 추가 메모리 절약 효과가 있다.

      결과적으로 NLL이 가장 낮은 후보를 최적화의 시작점(초기값)으로 선택한다.

    [2단계] L-BFGS-B 최적화
      - [1단계]에서 선정된 초기값에서 출발해 정밀 최적화 수행
      - 파라미터 범위 제약(α,δ,θ₁,θ₂ ∈ [0,1], μ_b,μ_s,ε_b,ε_s,Δ_b,Δ_s ≥ 0) 준수
...
■ 수치 안정성: log-space 연산
  ...
  ※ 기존 02_apin_weekly.py에서는 scipy.stats.poisson.pmf를 직접 곱하는 방식이었으나,
    이 코드에서는 log-space 연산으로 전환하여 수치 안정성을 크게 개선하였다.

  ※ 기존 02_apin_weekly.py의 시나리오 6번 가중치가 a*t1*d로 되어 있었으나,
    이는 a*t2*d가 올바르다 (호재+충격 시나리오이므로 정보일의 충격 확률 θ₂를 사용).
    이 코드에서는 올바르게 수정하여 구현하였다.

■ 병렬 처리 구조
  - multiprocessing.Pool: 종목 단위로 작업을 NUM_WORKERS개 코어에 분배
    (기본값: cpu_count - 4, 32 GB/16코어 환경 → 12 워커)
  - init_worker: 각 워커에 59,049×10 그리드 행렬을 공유 메모리로 한 번만 전달
...
■ 설정값 안내
  ...
  NUM_WORKERS            : 병렬 워커 프로세스 수 (기본 cpu_count - 4)
                           → 메모리 부족 시 줄일 것. 32 GB 환경에서 12 권장.
  GRID_CHUNK_SIZE        : 그리드 탐색 시 한 번에 처리할 행 수 (기본 8192)
...
# ── 메모리 및 병렬 처리 설정 ──
# APIN은 파라미터 10개로 그리드(59,049개)가 크고 워커당 메모리 소비가 높다.
# 아래 설정은 32 GB RAM 환경에 맞춰 조정되었으며,
# 사용 환경의 가용 메모리에 따라 NUM_WORKERS를 줄이면 안정성이 향상된다.
NUM_WORKERS           = max(1, multiprocessing.cpu_count() - 4)
                                # 전체 코어에서 4개를 OS·시스템에 예비로 남긴다.
                                # 32 GB / 16코어 환경 → 12 워커 사용.
                                # 워커당 피크 메모리 ≈ 120 MB이므로
                                # 12 워커 × 120 MB ≈ 1.4 GB만 추가 소비한다.
                                # 메모리 부족 시 8 또는 6으로 줄일 것.
GRID_CHUNK_SIZE       = 8192    # 그리드 탐색 시 한 번에 처리할 행 수.
                                # 청크당 피크 메모리 ≈ 3×C×N×8 바이트.
                                # 8192 × 60 × 8 × 3 ≈ 11 MiB (매우 안전)
...
# =============================================================================
#
# 아래 4개 함수는 '포아송 로그-PMF 계산'과 '우도 합산' 두 가지 역할을 담당한다.
#
#   _prefilter_grid : 모델 기대값으로 그리드를 빠르게 걸러냄 (59K → ~5K~9K)
#   _grid_search    : 필터링된 후보 중 NLL 최소 인덱스 반환 (최적 초기값 탐색)
...
def _prefilter_grid(grid: np.ndarray, B: np.ndarray, S: np.ndarray,
                    keep_ratio: float = 0.15,
                    min_keep: int = 500) -> tuple[np.ndarray, np.ndarray]:
    """
    59,049개 그리드 후보 중 관측 데이터와 명백하게 맞지 않는 후보를 빠르게 걸러낸다.

    원리:
      각 그리드 후보의 모델 기대값 E[B], E[S]를 산술적으로 계산하고,
      관측된 평균 B̄, S̄과 비교하여 거리(log 스케일)가 먼 후보를 제거한다.
      이 사전 필터링은 (G,) 1D 배열 연산만 사용하므로 윈도우당 < 1ms이다.

    Duarte & Young (2009) 모델의 기대값:
      E[B] = ε_b + α·δ·μ_b + (α·θ₂ + (1-α)·θ₁)·Δ_b
      E[S] = ε_s + α·(1-δ)·μ_s + (α·θ₂ + (1-α)·θ₁)·Δ_s

    필터링 기준:
      log-거리 = [ln(E[B]/B̄)]² + [ln(E[S]/S̄)]²
      거리가 작은 순으로 상위 keep_ratio(기본 15%)를 유지한다.
      최소 min_keep(기본 500)개는 항상 보존한다.

    효과:
      59,049 → 약 5,000~9,000개로 축소 (데이터에 따라 다름)
      → 이후 청크별 NLL 계산량이 약 7~10배 감소하여 큰 속도 개선.

    Args:
        grid      : shape (G, 10) — 전체 그리드
        B, S      : shape (N,)    — 윈도우 관측값
        keep_ratio: 유지할 비율 (기본 0.15 = 15%)
        min_keep  : 최소 유지 개수 (기본 500)
    Returns:
        filtered_grid    : shape (F, 10) — 필터링된 그리드
        original_indices : shape (F,)    — 원본 그리드에서의 인덱스 (결과 역매핑용)
    """
    G = grid.shape[0]
    keep_count = max(min_keep, int(G * keep_ratio))
    keep_count = min(keep_count, G)  # G보다 클 수 없음

    # 관측 평균 (0 방어)
    mean_B = max(np.mean(B), 1.0)
    mean_S = max(np.mean(S), 1.0)

    # 모델 기대값 — (G,) 1D 연산 (매우 빠름)
    a  = grid[:, 0]
    d  = grid[:, 1]
    t1 = grid[:, 2]
    t2 = grid[:, 3]
    ...
    shock_prob = a * t2 + (1 - a) * t1
    exp_B = eb + a * d * ub + shock_prob * pb
    exp_S = es + a * (1 - d) * us + shock_prob * ps

    # log-거리: 비율의 로그를 제곱하여 합산
    # 기대값이 0이면 inf가 되어 자연스럽게 탈락
    safe_exp_B = np.where(exp_B > 0, exp_B, 1e-300)
    safe_exp_S = np.where(exp_S > 0, exp_S, 1e-300)
    log_dist = (np.log(safe_exp_B / mean_B)) ** 2 + (np.log(safe_exp_S / mean_S)) ** 2

    # 거리가 작은 순으로 상위 keep_count개 선택
    if keep_count < G:
        # argpartition: O(G) — argsort O(G log G)보다 빠름
        top_indices = np.argpartition(log_dist, keep_count)[:keep_count]
    else:
        top_indices = np.arange(G)

    return grid[top_indices], top_indices


def _grid_search(grid: np.ndarray, B: np.ndarray, S: np.ndarray) -> int:
    """
    그리드 탐색으로 NLL이 최소인 파라미터 후보의 인덱스를 반환한다.

    3단계 최적화 전략:
      [1단계] 사전 필터링 (_prefilter_grid)
        각 그리드의 모델 기대값 E[B], E[S]를 관측 평균과 비교하여
        명백하게 맞지 않는 후보를 제거한다.
        59,049 → 약 5,000~9,000개로 축소. (1D 연산으로 < 1ms)

      [2단계] 청크 분할
        필터링된 그리드를 GRID_CHUNK_SIZE행 단위로 나눠 처리한다.
        피크 메모리가 그리드 전체 크기가 아닌 청크 크기에만 비례하므로
        워커당 메모리를 일정 수준으로 제한한다.

      [3단계] 수동 logsumexp
        6개 시나리오를 np.stack으로 (6, C, N) 텐서로 쌓는 대신,
        running max + running sum of exp로 하나씩 누적한다.
        3D 배열 할당이 완전히 제거된다.

    APIN 6가지 시나리오 (Duarte & Young 2009):
      시나리오   가중치                  매수 rate         매도 rate
      ─────────────────────────────────────────────────────────────
      1) 무정보+무충격   (1-α)(1-θ₁)         ε_b             ε_s
      2) 무정보+충격     (1-α)θ₁             ε_b+Δ_b         ε_s+Δ_s
      3) 악재+무충격     α(1-δ)(1-θ₂)       ε_b             μ_s+ε_s
      4) 악재+충격       α(1-δ)θ₂           ε_b+Δ_b         μ_s+ε_s+Δ_s
      5) 호재+무충격     αδ(1-θ₂)           μ_b+ε_b         ε_s
      6) 호재+충격       αδθ₂               μ_b+ε_b+Δ_b     ε_s+Δ_s
    ...
    Args:
        ...
        S    : shape (N,)    — 윈도우 매도 건수
    Returns:
        NLL이 최소인 **원본 그리드** 행의 인덱스
    """
    # ── [1단계] 사전 필터링: 59,049 → ~5,000~9,000 ──
    filtered_grid, original_indices = _prefilter_grid(grid, B, S)
    F = filtered_grid.shape[0]

    B_row = B.reshape(1, -1).astype(np.float64)  # (1, N)
    S_row = S.reshape(1, -1).astype(np.float64)

    # ln(k!) 를 청크마다 재계산하지 않도록 미리 한 번만 계산
    lgk_B = gammaln(B_row + 1.0)  # (1, N)
    lgk_S = gammaln(S_row + 1.0)  # (1, N)

    # 필터링된 그리드의 NLL을 담을 1D 배열
    all_nll = np.empty(F, dtype=np.float64)

    # ── [2단계] 청크 분할 + [3단계] 수동 logsumexp ──
    for start in range(0, F, GRID_CHUNK_SIZE):
        end = min(start + GRID_CHUNK_SIZE, F)
        g = filtered_grid[start:end]  # (C, 10)  C = chunk size
        ...
        # ── 인라인 log-포아송 PMF (함수 호출 오버헤드 제거) ──
        # 매수 rate 조합들의 safe rate — shape (C, 1)
        safe_eb       = np.where(eb_g > 0,             eb_g,             1e-300)
        safe_eb_pb    = np.where(eb_g + pb_g > 0,      eb_g + pb_g,     1e-300)
        safe_ub_eb    = np.where(ub_g + eb_g > 0,      ub_g + eb_g,     1e-300)
        safe_ub_eb_pb = np.where(ub_g + eb_g + pb_g > 0, ub_g + eb_g + pb_g, 1e-300)
        # 매도 rate 조합들
        safe_es       = np.where(es_g > 0,             es_g,             1e-300)
        safe_es_ps    = np.where(es_g + ps_g > 0,      es_g + ps_g,     1e-300)
        safe_us_es    = np.where(us_g + es_g > 0,      us_g + es_g,     1e-300)
        safe_us_es_ps = np.where(us_g + es_g + ps_g > 0, us_g + es_g + ps_g, 1e-300)

        # log-PMF 행렬: 각각 (C, N)
        lp_B_eb       = B_row * np.log(safe_eb)       - safe_eb       - lgk_B
        lp_B_eb_pb    = B_row * np.log(safe_eb_pb)    - safe_eb_pb    - lgk_B
        lp_B_ub_eb    = B_row * np.log(safe_ub_eb)    - safe_ub_eb    - lgk_B
        lp_B_ub_eb_pb = B_row * np.log(safe_ub_eb_pb) - safe_ub_eb_pb - lgk_B

        lp_S_es       = S_row * np.log(safe_es)       - safe_es       - lgk_S
        lp_S_es_ps    = S_row * np.log(safe_es_ps)    - safe_es_ps    - lgk_S
        lp_S_us_es    = S_row * np.log(safe_us_es)    - safe_us_es    - lgk_S
        lp_S_us_es_ps = S_row * np.log(safe_us_es_ps) - safe_us_es_ps - lgk_S

        # 시나리오별 하루 log-우도 (C, N) — B·S 독립이므로 log-PMF를 더함
        ...
        # 중간 PMF 배열 해제 — 이후에는 l0~l5만 필요
        del lp_B_eb, lp_B_eb_pb, lp_B_ub_eb, lp_B_ub_eb_pb
        del lp_S_es, lp_S_es_ps, lp_S_us_es, lp_S_us_es_ps

        # 가중치 ln 변환 — shape (C, 1), 브로드캐스팅으로 (C, N)에 더해짐
        lw0 = np.log(np.clip((1 - a_g) * (1 - t1_g),        1e-300, None))
        lw1 = np.log(np.clip((1 - a_g) * t1_g,              1e-300, None))
        lw2 = np.log(np.clip(a_g * (1 - d_g) * (1 - t2_g),  1e-300, None))
        lw3 = np.log(np.clip(a_g * (1 - d_g) * t2_g,        1e-300, None))
        lw4 = np.log(np.clip(a_g * d_g * (1 - t2_g),        1e-300, None))
        lw5 = np.log(np.clip(a_g * d_g * t2_g,              1e-300, None))

        # ── 수동 logsumexp (3D 배열 할당 제거) ──
        # 6개 시나리오의 weighted log-likelihood를 하나씩 순회하며
        # running max와 running sum of exp를 유지한다.
        #
        #   logsumexp([x₀, x₁, ..., x₅])
        #   = max_x + ln(Σ exp(xᵢ - max_x))
        #
        # 이 방식은 (6, C, N) 텐서를 만들지 않으므로
        # 피크 메모리가 약 3×(C, N)으로 일정하다.
        a_max   = lw0 + l0                          # (C, N) — 첫 번째 항
        sum_exp = np.ones_like(a_max)                # exp(0) = 1

        for lw_i, l_i in [(lw1, l1), (lw2, l2), (lw3, l3), (lw4, l4), (lw5, l5)]:
            term = lw_i + l_i                        # (C, N)
            new_max = np.maximum(a_max, term)
            sum_exp = sum_exp * np.exp(a_max - new_max) + np.exp(term - new_max)
            a_max   = new_max

        log_lik_per_day = a_max + np.log(sum_exp)   # (C, N)

        # 날짜 축 합산 → 청크별 NLL: (C,)
        all_nll[start:end] = -np.sum(log_lik_per_day, axis=1)

    # 필터링된 그리드 내 최적 → 원본 그리드 인덱스로 역매핑
    best_filtered_idx = int(np.argmin(all_nll))
    return int(original_indices[best_filtered_idx])
...
def _make_nll(B: np.ndarray, S: np.ndarray):
    ...
    내부 함수 negative_log_likelihood(params):
      파라미터 배열 [α, δ, θ₁, θ₂, μ_b, μ_s, ε_b, ε_s, Δ_b, Δ_s]를 받아
      NLL 스칼라를 반환한다.
    ...
    def negative_log_likelihood(params):
        ...
        # 시나리오별 하루 log-우도
        l0 = log_pB_eb       + log_pS_es          # 1) 무정보 + 무충격
        l1 = log_pB_eb_pb    + log_pS_es_ps       # 2) 무정보 + 충격
        l2 = log_pB_eb       + log_pS_us_es       # 3) 악재   + 무충격
        l3 = log_pB_eb_pb    + log_pS_us_es_ps    # 4) 악재   + 충격
        l4 = log_pB_ub_eb    + log_pS_es          # 5) 호재   + 무충격
        l5 = log_pB_ub_eb_pb + log_pS_es_ps       # 6) 호재   + 충격
        ...
        # logsumexp 트릭: shape (6, N) → axis=0으로 축소 → (N,)
        log_terms = np.stack([
            math.log(w0) + l0,
            math.log(w1) + l1,
            math.log(w2) + l2,
...
# =============================================================================
# [Step 2] APIN 추정 통합 함수
# =============================================================================

def estimate_apin_parameters(B_array: np.ndarray, S_array: np.ndarray,
                              grid_combinations: np.ndarray) -> dict:
    """
    60일 윈도우의 B, S 배열을 받아 Duarte & Young(2009) MLE로
    10개 파라미터와 APIN, PSOS를 추정한다.

    추정 절차:
      1. _grid_search로 59,049개 후보 중 NLL 최소 후보를 초기값으로 선택
      2. L-BFGS-B로 정밀 최적화 (파라미터 범위 제약 적용)
      3. 수렴 성공 시 APIN, PSOS 계산
    ...
    # APIN / PSOS 계산식 (Duarte & Young 2009)
    # 분모 (Total Expected Order Flow)
    ...
    return {
        ...
        "APIN": apin, "PSOS": psos, "converged": True,
    }
...
GLOBAL_GRID     = None   # 59,049×10 그리드 행렬
...
def process_single_symbol(data_tuple: tuple) -> list[dict]:
    ...
    for i in range(WINDOW_SIZE - 1, n):
        ...
        est = estimate_apin_parameters(window_B, window_S, GLOBAL_GRID)
...
def run_apin_calculation(daily_bs_path: str, output_dir: str,
    ...
    session_dir      = os.path.join(intermediate_dir, f"session_{run_id}")
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(session_dir, exist_ok=True)

    print(f"\n{'='*65}")
    print(f"[Step 2] APIN 계산 시작")
    print(f"{'='*65}")
    ...
    # 3. 그리드 행렬 생성 (3^10 = 59,049개 파라미터 조합)
    #    순서: α, δ, θ₁, θ₂, μ_b, μ_s, ε_b, ε_s, Δ_b, Δ_s
    print("\n[Grid 초기값 생성 중...]")
    grid_matrix = np.array(
        list(itertools.product(
            [0.1, 0.5, 0.9],   # α   (정보 이벤트 확률)
            [0.1, 0.5, 0.9],   # δ   (호재 확률)
            [0.1, 0.5, 0.9],   # θ₁  (무정보일 충격 확률)
            [0.1, 0.5, 0.9],   # θ₂  (정보일 충격 확률)
            [20, 200, 2000],   # μ_b (호재 정보 매수)
            [20, 200, 2000],   # μ_s (악재 정보 매도)
            [20, 200, 2000],   # ε_b (비정보 매수)
            [20, 200, 2000],   # ε_s (비정보 매도)
            [20, 200, 2000],   # Δ_b (충격 추가 매수)
            [20, 200, 2000],   # Δ_s (충격 추가 매도)
        )),
        dtype=np.float64,
    )
    print(f"  Grid 크기   : {grid_matrix.shape[0]:,} combinations")
    print(f"  사전필터링  : 기대값 기반 상위 ~15% 유지 (→ ~{max(500, int(grid_matrix.shape[0]*0.15)):,}개)")

    num_cores = NUM_WORKERS
    print(f"  CPU 코어    : {multiprocessing.cpu_count()}개 중 {num_cores}개 사용"
          f"  (OS 예비 {multiprocessing.cpu_count() - num_cores}개)")
    print(f"  Grid 청크   : {GRID_CHUNK_SIZE} rows/chunk")
