"""
=============================================================================
[통합 파이프라인] 틱 데이터 전처리 및 일별 롤링 APIN 계산 — v03 최적화
=============================================================================

■ 기존(01기본) 대비 변경 사항 요약

  [변경 1] 비율(Multiplier) 방어막이 적용된 동적 그리드 (Dynamic Grid)
    - 기존: run_apin_calculation에서 3^10 = 59,049개 고정 그리드를 전역 생성 후
            init_worker로 각 워커에 전달 → 대형 (59049, N) 텐서 연산으로 OOM 발생
    - 변경: 전역 그리드를 완전히 제거하고, 각 윈도우의 B/S 비영(>0) 관측값의
            분위수와 Multiplier 하한·상한을 결합하여 2^10 = 1,024개 조합을 동적 생성
            → 메모리 사용량 약 58배 감소 (59,049 → 1,024)
    - 비율 방어막: 윈도우가 잔잔하여 상위 분위수 - 중앙값 ≈ 0이 되는 경우에도
            중앙값의 배수(0.5×, 3.0×)로 하한/상한을 보장하여 탐색 범위를 유지
    - 안정성 분기: B/S 각 측의 비영 관측일이 20일 미만이면
            분위수가 불안정하므로 고정 폴백 [100, 1000]을 사용

  [변경 2] Top-3 멀티 스타트 (Multi-Start) 최적화
    - 기존: 그리드에서 NLL 최소 1개만 선택 → L-BFGS-B 1회 수행
    - 변경: 그리드에서 NLL 상위 3개를 선택 → L-BFGS-B를 독립 3회 수행
            → 수렴한 결과 중 최종 NLL이 가장 낮은 것을 채택
    - 효과: 초기값 1개에 의존하던 지역 최적해 함정을 3개 출발점으로 분산 회피

  [변경 3] 조건부 웜 스타트 (Warm Start) + 주기적 리셋
    - 이전 윈도우 최적화 성공 시: 동적 그리드 + Top-3를 모두 생략하고
      이전 최적 파라미터 1개만 x0로 투입 → L-BFGS-B 1회 수행 (극한 속도)
    - 20회 연속 웜 스타트 성공 시: 강제로 동적 그리드 + Top-3 수행 (리셋)
    - 유효일 부족 스킵 또는 수렴 실패 시: 웜 스타트 즉시 초기화
...
■ APIN 모델 (Duarte & Young 2009)
  ...
  자세한 수식 설명은 01기본.py의 docstring 참조.

■ MLE 추정 방식 (v03 기준)
  [1단계-A] 동적 그리드 + Top-3 서치 (그리드 모드)
    윈도우 B/S 분위수 기반 2^10 = 1,024개 후보 생성 후
...
■ 설정값 안내
  ...
  DYNAMIC_GRID_MIN_NONZERO : 동적 그리드 안정성 기준 비영 관측일 수 (기본 20)
...
# 동적 그리드 안정성 판단 기준: 비영(非零) 관측일 최소 개수
#
# 윈도우 60일 중 B>0 또는 S>0인 날이 이 값 미만이면 해당 측(매수/매도)의
# 분위수 기반 후보값이 불안정하다고 판단하여 고정 폴백 [100, 1000]을 사용한다.
#
# 기준값 20일의 근거:
#   - 분위수 계산의 통계적 안정성: percentile(5)와 percentile(95)가 의미 있으려면
#     비영 표본이 최소 20개는 필요하다. 20개 미만이면 5%ile ≈ 최솟값, 95%ile ≈ 최댓값이
#     되어 개별 이상치에 극도로 민감해진다.
#   - WINDOW_SIZE(60) 대비 비율: 60일 중 20일 = 1/3.
#     이보다 적으면 해당 종목이 해당 기간에 거의 거래되지 않았다는 의미이므로
#     데이터에서 분포를 추론하는 것 자체가 위험하다.
#   - 고정 폴백 [100, 1000]: 한국 주식 시장 틱 데이터의 일반적 일별 거래 건수 스케일에서
#     100은 소형주 평균, 1000은 중형주 평균에 해당하여 넓은 탐색 범위를 제공한다.
DYNAMIC_GRID_MIN_NONZERO = 20
...
# =============================================================================
# [Step 2 — v03 신규] 비율(Multiplier) 방어막이 적용된 동적 그리드 생성
# =============================================================================

def generate_dynamic_grid(B: np.ndarray, S: np.ndarray) -> np.ndarray:
    """
    현재 60일 윈도우의 B/S 데이터 분포를 분석하여
    2^10 = 1,024개의 초기값 조합을 동적으로 생성한다.

    ■ 핵심 설계 원리
      기존 3^10 = 59,049개 고정 그리드는 데이터 특성과 무관하게 넓은 범위를
      탐색하므로 (59049, 60) 크기의 텐서 연산이 필요했다.
      이를 윈도우 데이터의 분위수 + 비율(Multiplier) 방어막으로 대체하면:
        (1) 그리드 크기 58배 감소 → OOM 해소
        (2) 데이터에 맞는 초기값 → 수렴 속도 향상
        (3) 잔잔한 윈도우에서도 탐색 범위 보장 → 지역 최적해 방지

    ■ 안정성 분기
      매수(B) / 매도(S) 각 측에 대해 비영(>0) 관측일 수를 센다.
      DYNAMIC_GRID_MIN_NONZERO(20일) 이상이면 분위수 기반 동적 후보값을 사용하고,
      미만이면 고정 폴백 [100, 1000]을 사용한다.

      분기 조건을 B와 S에 독립 적용하는 이유:
        유효 거래일(B+S > 0)이 30일 이상이어도 한쪽만 편중될 수 있다.
        예) 악재 국면에서 B>0 = 18일, S>0 = 35일 → B 측만 폴백, S 측은 동적.

    ■ 파라미터별 후보값 설정 논리 (안정 모드, 비영 관측값 기반)

      확률 파라미터 (a, d, t1, t2):
        [0.3, 0.7] 고정. L-BFGS-B가 경계 근처까지 자유롭게 탐색하므로
        출발점은 중간 대역이 효율적.

      엡실론 (eb, es) — 비정보 거래자 기본 도착률:
        비영 관측값만 추출하여 [하위 5%, 중앙값]을 후보로 사용.
        하위 5% → 거래가 가장 적은 날의 기본 수준
        중앙값  → 평균적 비정보 거래 수준

      뮤 (ub, us) & 델타 (pb, ps) — 정보/충격 추가 도착률:
        ※ 비율(Multiplier) 방어막 핵심 논리
        정보 이벤트·충격이 터진 날은 평소보다 거래가 급증하므로
        '상위 꼬리와 중앙값의 갭'이 좋은 추정값이다. 그러나 윈도우 전체가
        잔잔하면(종목이 거의 등락 없이 박스권이면) 분위수 갭이 ≈ 0이 된다.
        이 경우 뮤·델타 초기값이 0 근처에서 시작하면 L-BFGS-B가 정보 거래가
        없는 것으로 잘못 수렴할 위험이 있다.

        방어 전략 — max(분위수 갭, 중앙값 × 배수):
          Low 후보  = max(p90 − 중앙값,        중앙값 × 0.5)
          High 후보 = max(최대값 − p05,         중앙값 × 3.0)

        이렇게 하면:
          - 변동성 충분: 분위수 갭이 배수 하한보다 크면 데이터 기반 값 사용
          - 변동성 부족: 최소한 중앙값의 0.5배(보수적) ~ 3배(공격적) 범위가 보장
          - 중앙값 자체가 0인 극단 케이스: max(..., 1.0) 최종 보정으로 방어
    ...
    b_stable = len(B_nz) >= DYNAMIC_GRID_MIN_NONZERO
    s_stable = len(S_nz) >= DYNAMIC_GRID_MIN_NONZERO
    ...
    # ── 매수 측 (eb, ub, pb) ───────────────────────────────────────────────
    if b_stable:
        b_p05 = np.percentile(B_nz, 5)
        b_med = np.median(B_nz)
        b_p90 = np.percentile(B_nz, 90)
        b_max = np.max(B_nz)
        ...
        # 뮤 & 델타: 비율 방어막 적용
        #   Low  = max(p90 - 중앙값,       중앙값 × 0.5)   ← 보수적
        #   High = max(최대값 - p05,        중앙값 × 3.0)   ← 공격적
        ub_low  = max(b_p90 - b_med,  b_med * 0.5,  1.0)
        ub_high = max(b_max - b_p05,  b_med * 3.0,  1.0)
        ub_candidates = [ub_low, ub_high]
        pb_candidates = [ub_low, ub_high]    # Δ_b ≈ μ_b 규모
    else:
        # 폴백: 비영 관측일 < 20일 → 분위수 불안정
        eb_candidates = [100.0, 1000.0]
        ub_candidates = [100.0, 1000.0]
        pb_candidates = [100.0, 1000.0]

    # ── 매도 측 (es, us, ps) ───────────────────────────────────────────────
    if s_stable:
        s_p05 = np.percentile(S_nz, 5)
        s_med = np.median(S_nz)
        s_p90 = np.percentile(S_nz, 90)
        s_max = np.max(S_nz)
        ...
        us_low  = max(s_p90 - s_med,  s_med * 0.5,  1.0)
        us_high = max(s_max - s_p05,  s_med * 3.0,  1.0)
        us_candidates = [us_low, us_high]
        ps_candidates = [us_low, us_high]
    else:
        es_candidates = [100.0, 1000.0]
        us_candidates = [100.0, 1000.0]
        ps_candidates = [100.0, 1000.0]
...
def process_single_symbol(data_tuple: tuple) -> list[dict]:
    ...
    for i in range(WINDOW_SIZE - 1, n):
        ...
        if use_warm_start:
            ...
            # ▶ [모드 A] 풀 서치: 동적 그리드 생성 + Top-3 멀티 스타트
            # (주기적 리셋이었다면 streak 카운터도 0으로 초기화)
            dynamic_grid = generate_dynamic_grid(window_B, window_S)
            top_k_indices = _grid_search_topk(dynamic_grid, window_B, window_S,
                                              k=TOP_K)
            top_k_x0s = [dynamic_grid[idx] for idx in top_k_indices]
            warm_streak = 0
...
def run_apin_calculation(daily_bs_path: str, output_dir: str,
    ...
    v03 변경사항:
      - 59,049개 고정 그리드 행렬 생성 로직 완전 제거
      - init_worker에 calendar만 전달 (grid 제거)
      - 동적 그리드 + Top-K + 웜 스타트는 process_single_symbol 내부에서 처리
    """
    ...
    print(f"\n{'='*65}")
    print(f"[Step 2] APIN 계산 시작 (v03: 동적 그리드 + Top-{TOP_K} + 웜 스타트)")
    print(f"{'='*65}")
    ...
    # v03: 고정 그리드 제거 → 동적 그리드는 process_single_symbol 내부에서 생성
    print(f"\n[v03 최적화 모드]")
    print(f"  그리드 크기          : 윈도우별 동적 2^10 = 1,024개")
    print(f"  멀티 스타트          : Top-{TOP_K}")
    print(f"  웜 스타트 리셋 간격  : {WARM_RESET_INTERVAL}회")
    print(f"  동적 그리드 안정 기준: 비영 관측일 ≥ {DYNAMIC_GRID_MIN_NONZERO}일")
