...
실행 순서:
  1) python 00pin/01_preprocess.py
  2) Rscript 00pin/02_r_pin.R
...
def _parse_folder(folder: str):
    ...
    return m.group(1), m.group(2), m.group(3)  # country, start, end
...
if __name__ == "__main__":
    start = datetime.now()
    print(f"\n{'='*65}")
    print(f"[PIN 전처리] {start.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    print(f"\n다음 단계: Rscript 00pin/02_r_pin.R")
